...
export function RangeBars({ items, min, max, ticks = [], height }: RangeBarsProps) {
  const W = 900, padL = 150, padR = 16, padT = 8, padB = 24;
  const rowH = 26;
      ...
      {items.map((it, i) => {
        ...
        return (
          <g key={it.label}>
            ...
            <rect x={x(it.start)} y={y + 5} width={Math.max(x(it.end) - x(it.start), 2)}
                  height={rowH - 12} rx={3} fill="var(--forest)" opacity={0.75} />
            {it.mark != null && (
              <line x1={x(it.mark)} x2={x(it.mark)} y1={y + 2} y2={y + rowH - 4}
