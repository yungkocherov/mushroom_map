...
**Бренд.** Переименование: `mushroom-map` → **geobiom** (рабочее имя, требует явного подтверждения перед стартом фазы 1 — домен `geobiom.ru` нужно проверить на доступность; если занят — выбрать альтернативу. До подтверждения — все упоминания `geobiom`/`staging.geobiom.ru`/OG-тегов в спеке считаются плейсхолдером бренд-имени.). Tagline — *«лес ленобласти»*.
...
- [ ] **Подтвердить бренд-имя** — `geobiom` остаётся, или альтернатива? Это блокирующее решение для фазы 1 (нужно для OG-тегов, домена, копи в нав-баре).
- [ ] **`.gitignore` check** — убедиться, что `.superpowers/` уже в gitignore (если нет — добавить).
- [ ] **ExitPlanMode** + commit + push спека + всех brainstorm-мокапов в `.superpowers/brainstorm/` → `origin/main`.
