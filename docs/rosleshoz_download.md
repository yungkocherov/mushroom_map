...
> **Status: superseded** (2026-05-04). MVT-flow (Вариант 0) даёт ~70%
> покрытия и удалён из репо. Текущий путь — bulk-скрап через ФГИС API
> (`pipelines/scrape_fgislk_attrinfo.py` + memory `reference_fgislk_api.md`).
> Документ оставлен как референс по альтернативным вариантам (WFS, GeoTIFF,
> open-data порталам) и истории попыток.
