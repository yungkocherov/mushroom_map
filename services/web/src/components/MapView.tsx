...
import { FOREST_LAYER_PAINT } from "../lib/forestStyle";
...
function addForestLayer(m: Map) {
  ...
  m.addLayer({
    ...
    source: "forest",
    "source-layer": "forest",
    paint: { "line-color": "#00000030", "line-width": 0.8 },
    minzoom: 10,
  });
}
...
export function MapView() {
  ...
  useEffect(() => {
    ...
    // Пробуем загрузить векторный стиль; при ошибке — inline fallback
    ...
    // При успешной загрузке стиля
    m.on("load", () => addForestLayer(m));

    // Если внешний стиль не загрузился — fallback на inline
    m.on("error", (e) => {
      // Проверяем что ошибка связана со стилем (не с данными)
      if (
        ...
        m.once("styledata", () => addForestLayer(m));
      }
    });
