...
def encode_oauth_state(payload: dict[str, Any]) -> str:
    ...
    Используется для CSRF-state в Yandex OAuth flow. Также бережёт
    `code_challenge` (PKCE S256) — на callback мы пересчитываем
    challenge из verifier-cookie и сравниваем со state-payload.
    Без этого binding'а атакующий может подменить только PKCE-cookie
    и привязать жертву к своей Yandex-учётке.

    Подписывается отдельным `oauth_state_secret` если задан, иначе
    fallback к `jwt_secret` (см. settings.effective_oauth_state_secret).
    """
    now = int(time.time())
    jwt_payload = {
        **payload,
    ...
    return jwt.encode(jwt_payload, settings.effective_oauth_state_secret, algorithm=_ALGO)
