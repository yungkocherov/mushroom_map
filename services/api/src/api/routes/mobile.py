...
def _read_regions_manifest() -> Optional[dict]:
    """Чтение `<tiles_dir>/regions.json` (генерится pipeline'ом
    `pipelines/build_district_tiles.py`). Возвращает None если файла
    нет — endpoint в этом случае отдаёт 503 и фронт показывает
    «download manager недоступен, попробуй позже».
    """
    import json
    from api.settings import settings

    manifest_path = Path(settings.tiles_dir) / "regions.json"
    ...
    try:
        ...
        log.warning("regions.json unreadable: %s", exc)
        return None
...
def list_regions() -> RegionsResponse:
    ...
    Читает `data/tiles/regions.json` (генерится
    `pipelines/build_district_tiles.py`). Mobile-app сравнивает
    `manifest_version` каждого района с локально сохранённым; если
    отличается — предлагает обновить регион.

    Если manifest отсутствует (pipeline не запускался) — 503.
    """
    manifest = _read_regions_manifest()
    if manifest is None:
        raise HTTPException(
            status_code=503,
            detail=(
                "regions manifest not available — run "
                "pipelines/build_district_tiles.py to generate it"
            ),
        )
    return RegionsResponse(
        version=manifest.get("version", ""),
        base_url=manifest.get("base_url", ""),
        regions=[RegionDto(**r) for r in manifest.get("regions", [])],
    )
