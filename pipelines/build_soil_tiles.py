"""
build_soil_tiles: generate soil.pmtiles from the soil_polygon table.

Usage:
    python pipelines/build_soil_tiles.py --region lenoblast
"""
    ...
    row = conn.execute(
        ...
        WITH mvt_src AS (
            SELECT
                ...
                COALESCE(t.descript, '') AS descript,
                COALESCE(t.zone, '')     AS zone,
                ST_AsMVTGeom(
            ...
            FROM soil_polygon p
            LEFT JOIN soil_type t ON t.soil_id = p.soil0_id
            WHERE ST_Transform(p.geometry, 3857) && ST_TileEnvelope(%s, %s, %s)
        )
        SELECT ST_AsMVT(mvt_src, 'soil', %s, 'geom')
        FROM mvt_src
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build soil.pmtiles from soil_polygon table")
    ...
    ap.add_argument("--out",     default="data/tiles/soil.pmtiles")
    ...
    with psycopg.connect(dsn, autocommit=True) as conn:
        ...
        n_rows = conn.execute("SELECT COUNT(*) FROM soil_polygon").fetchone()[0]
        print(f"soil_polygon rows: {n_rows}")
        ...
        with open(tmp_path, "wb") as f:
            ...
            for z in range(args.minzoom, args.maxzoom + 1):
                ...
                print(f"  z={z:<2d}  tiles={n_tiles:<5d}  ok={z_ok}  ({dt:.1f}s)")
            ...
            metadata = {
                "name": f"mushroom-map soil {args.region}",
                "vector_layers": [{
                    "id": "soil",
                    "fields": {
                        "poligon_id": "Number",
